...
async function createTransaction(tx, uid, type, amount, balanceBefore, description, battleId = null) {
  const ref = db.collection('transactions').doc()
  await tx.set(ref, {
    uid, type, amount,
    balanceBefore,
    balanceAfter: type === 'credit' || type === 'bonus' ? balanceBefore + amount : balanceBefore - amount,
    description,
    battleId,
...
// ─── 1. onBattleJoined ────────────────────────────────────────────────────────
exports.onBattleJoined = onDocumentUpdated('battles/{battleId}', async (event) => {
  ...
  await db.runTransaction(async (tx) => {
    ...
    const [creatorSnap, joinerSnap] = await Promise.all([tx.get(creatorRef), tx.get(joinerRef)])
    ...
    tx.update(creatorRef, { walletBalance: FieldValue.increment(-entryFee) })
    tx.update(joinerRef,  { walletBalance: FieldValue.increment(-entryFee) })
    tx.update(battleRef,  { status: 'running', startedAt: FieldValue.serverTimestamp() })

    await createTransaction(tx, creatorId, 'debit', entryFee, creatorBal, `Battle entry fee`, battleId)
    await createTransaction(tx, joinerId,  'debit', entryFee, joinerBal,  `Battle entry fee`, battleId)
  })

  await sendNotification(creatorId, 'Battle Started!', `${after.joinerName} joined your battle. Good luck!`)
  await sendNotification(joinerId,  'Battle Started!', `You joined ${after.creatorName}'s battle. Good luck!`)
})

// ─── 2. onRoomCodeSet ─────────────────────────────────────────────────────────
exports.onRoomCodeSet = onDocumentUpdated('battles/{battleId}', async (event) => {
  const before = event.data.before.data()
  const after  = event.data.after.data()
...
exports.onRoomDeadline = onCall({ region: 'asia-south1' }, async (request) => {
  ...
  await db.runTransaction(async (tx) => {
    ...
    const [cs, js]   = await Promise.all([tx.get(creatorRef), tx.get(joinerRef)])
    ...
    await createTransaction(tx, data.creatorId, 'credit', data.prizePool, cs.data().walletBalance, 'Battle won (forfeit)', battleId)
  })
...
exports.onResultSubmitted = onDocumentUpdated('battles/{battleId}', async (event) => {
  ...
  const { creatorId, joinerId, creatorResult, joinerResult, prizePool, entryFee } = after
  ...
  if (
    ...
    await db.runTransaction(async (tx) => {
      ...
      const battleRef = db.collection('battles').doc(battleId)
      const [ws, ls]  = await Promise.all([tx.get(winnerRef), tx.get(loserRef)])
      ...
      await createTransaction(tx, winnerId, 'credit', prizePool, ws.data().walletBalance, `Battle won`, battleId)
    })
...
async function verifyAdmin(uid) {
  const snap = await db.collection('users').doc(uid).get()
  if (!snap.exists() || !snap.data().isAdmin) throw new HttpsError('permission-denied', 'Admin only')
  return snap.data()
}
...
exports.onDepositApproved = onCall({ region: 'asia-south1' }, async (request) => {
  ...
  await db.runTransaction(async (tx) => {
    ...
    tx.update(reqRef, { status: 'approved', resolvedAt: FieldValue.serverTimestamp() })

    await createTransaction(tx, uid, 'credit', amount, bal, `Deposit approved`, null)
    ...
    if (referredBy && !userSnap.data().firstDepositDone) {
      ...
      if (refSnap.exists()) {
        ...
        await createTransaction(tx, referredBy, 'bonus', 10, refSnap.data().walletBalance, 'Referral bonus', null)
      }
    }
  })

  await sendNotification(uid, 'Deposit Approved!', `₹${amount} has been added to your wallet.`)
  return { success: true }
})

// ─── 6. onDepositRejected ─────────────────────────────────────────────────────
exports.onDepositRejected = onCall({ region: 'asia-south1' }, async (request) => {
  await verifyAdmin(request.auth?.uid)
  ...
  await reqRef.update({ status: 'rejected', adminNote: adminNote || '', resolvedAt: FieldValue.serverTimestamp() })
  await sendNotification(uid, 'Deposit Rejected', adminNote || 'Your deposit request was rejected.')
  return { success: true }
})

// ─── 7. onWithdrawalApproved ──────────────────────────────────────────────────
exports.onWithdrawalApproved = onCall({ region: 'asia-south1' }, async (request) => {
  await verifyAdmin(request.auth?.uid)
  ...
  await db.runTransaction(async (tx) => {
    ...
    tx.update(userRef, { walletBalance: FieldValue.increment(-amount) })
    tx.update(reqRef, { status: 'approved', resolvedAt: FieldValue.serverTimestamp() })
    await createTransaction(tx, uid, 'debit', amount, bal, 'Withdrawal processed', null)
  })

  await sendNotification(uid, 'Withdrawal Processed', `₹${amount} has been sent to your UPI.`)
  return { success: true }
})

// ─── 8. onDisputeResolved ─────────────────────────────────────────────────────
exports.onDisputeResolved = onCall({ region: 'asia-south1' }, async (request) => {
  await verifyAdmin(request.auth?.uid)
  ...
  await db.runTransaction(async (tx) => {
    if (decision === 'refund_both') {
      ...
      tx.update(c, { walletBalance: FieldValue.increment(battle.entryFee) })
      tx.update(j, { walletBalance: FieldValue.increment(battle.entryFee) })
      tx.update(battleRef, { status: 'completed', adminVerified: true, result: 'disputed', completedAt: FieldValue.serverTimestamp() })
      await createTransaction(tx, battle.creatorId, 'refund', battle.entryFee, cs.data().walletBalance, 'Dispute refund', battleId)
      await createTransaction(tx, battle.joinerId,  'refund', battle.entryFee, js.data().walletBalance, 'Dispute refund', battleId)
    } else {
      ...
      tx.update(winnerRef, { walletBalance: FieldValue.increment(battle.prizePool), totalWins: FieldValue.increment(1) })
      tx.update(db.collection('users').doc(loserId), { totalLosses: FieldValue.increment(1) })
      tx.update(battleRef, { status: 'completed', adminVerified: true, winnerId, winnerName, result: decision === 'player1_wins' ? 'creator_won' : 'joiner_won', completedAt: FieldValue.serverTimestamp() })
      await createTransaction(tx, winnerId, 'credit', battle.prizePool, ws.data().walletBalance, 'Dispute resolved — won', battleId)
    }
  })
