export function Button({ children, variant = 'primary', className = '', disabled, loading, ...props }) {
  const base = 'btn-press inline-flex items-center justify-center gap-2 rounded-[6px] font-display font-semibold transition-all duration-150 disabled:opacity-40 disabled:cursor-not-allowed'
  const variants = {
    primary:  'bg-green text-black hover:bg-green-d px-4 py-3 text-sm',
    outline:  'border border-border2 text-text2 hover:border-green hover:text-green px-4 py-3 text-sm',
