...
import BattlesList from "../components/ui/BattlesList";
...
export default function Home() {
	...
	const { openBattles, myBattles, setOpenBattles, setMyBattles } =
		useBattleStore();
	const [activeFee, setActiveFee] = useState(null);
	const [joiningId, setJoiningId] = useState(null);
	const navigate = useNavigate();

	const listFilterByPriceMoney = activeFee
		? openBattles.filter(
				(b) => b.entryFee === activeFee && b.creatorId !== user?.uid,
			)
		: openBattles.filter((b) => b.creatorId !== user?.uid);

	useEffect(() => {
		const u1 = watchOpenBattles((snap) =>
			setOpenBattles(snap.docs.map((d) => ({ id: d.id, ...d.data() }))),
		);
		const u2 = watchMyBattles(user?.uid, (docs) =>
			setMyBattles(docs.map((d) => ({ id: d.id, ...d.data() }))),
		);
		return () => {
			u1();
			u2();
			...
			<div className="px-4 pt-4 space-y-5 pb-6">
				...
				<div className="bg-surface border border-border rounded-[8px] p-4">
					<div className="flex items-start justify-between">
						<div>
							<p className="font-mono text-3xl font-bold text-text1">
								{openBattles.length}
							</p>
							<p className="text-[11px] uppercase tracking-widest text-text3 font-semibold mt-0.5">
								Open Battles
							</p>
				...
				<div>
					<div className="flex items-center gap-2 mb-3">
						...
						{listFilterByPriceMoney.length > 0 && (
							<span className="w-5 h-5 rounded-full bg-red flex items-center justify-center text-[10px] font-bold text-white">
								{listFilterByPriceMoney.length}
							</span>
						)}
					</div>
					{listFilterByPriceMoney.length === 0 ? (
				...
				{myBattles.length > 0 && (
					<div>
						<p className="text-[11px] uppercase tracking-widest text-text3 font-semibold mb-3">
							My Active Battles
						</p>
