...
import { watchBattle, updateBattle } from '../../firebase/firestore'
      ...
      <div className="px-4 pt-4 space-y-4 pb-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button onClick={() => navigate('/home')} className="w-9 h-9 bg-surface border border-border rounded-[6px] flex items-center justify-center text-text2">
              <ChevronLeft size={18} />
            </button>
            <div>
        ...
        <div className="bg-surface border border-border rounded-[8px] overflow-hidden">
          ...
          <div className="flex items-center justify-between p-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="font-display font-semibold text-sm text-text1">
                  {!battle.joinerId ? 'Waiting for opponent' : isCreator ? battle.joinerName : `You (${profile?.username})`}
                </p>
        ...
        {isCreator && !battle.roomCode && battle.status === 'running' && (
          <div className="bg-surface border border-border rounded-[8px] p-4 space-y-3">
            ...
            <input
              ...
              maxLength={8}
            />
            <p className="text-[11px] text-text3">Create a room in Ludo King and enter the code here</p>
            <Button variant="primary" className="w-full" loading={loading} disabled={!roomCodeInput.trim()} onClick={setRoomCode}>
              Share Room Code
            </Button>
          </div>
        )}
        ...
        {!isCreator && !battle.roomCode && battle.status === 'running' && (
          <div className="bg-surface border border-border rounded-[8px] p-6 flex flex-col items-center gap-3">
            ...
            <p className="text-text3 text-xs text-center">The creator will share the Ludo King room code shortly</p>
          </div>
        )}
        ...
        {battle.joinerJoined && battle.status === 'running' && (
          <div className="bg-surface border border-border rounded-[8px] p-6 text-center space-y-4">
            ...
            <Button variant="primary" className="w-full" onClick={() => navigate(`/battle/${battleId}/result`)}>
              Submit Match Result
            </Button>
          </div>
        )}
