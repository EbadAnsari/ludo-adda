...
import { calcPlatformFee, calcPrize, formatAmount } from "../../utils/currency";
import { createCurrentTimeStamp } from "../../utils/time";

const AMOUNTS = [50, 100, 200, 500, 1000] as const;

export default function CreateBattle() {
	const [selected, setSelected] = useState<
		(typeof AMOUNTS)[number] | number | null
	>(null);
	...
	async function handleCreate() {
		if (!selected) return;
		if (profile.walletBalance < selected) {
		...
		try {
			const ref = await createBattle({
				...
				postedAt: createCurrentTimeStamp(),
			});
		...
		<PageWrapper>
			<div className="px-4 pt-4 space-y-6 pb-6">
				<div className="flex items-center gap-3">
					<button
						onClick={() => navigate(-1)}
						className="w-9 h-9 bg-surface border border-border rounded-[6px] flex items-center justify-center text-text2 hover:text-text1 transition-colors"
					>
						<ChevronLeft size={18} />
					</button>
					<h1 className="font-display font-bold text-xl text-text1">
						New Battle
					</h1>
				</div>
					...
					<div className="bg-surface border border-border rounded-[8px] p-4">
						<div className="grid grid-cols-3 divide-x divide-border text-center">
							<div className="pr-3">
								...
								<p className="font-mono font-bold text-text1 mt-1">
							...
							<div className="px-3">
								...
								<p className="font-mono font-bold text-gold mt-1">
									{formatAmount(calcPrize(selected))}
								</p>
							</div>
							<div className="pl-3">
								<p className="text-[11px] uppercase tracking-widest text-text3 font-semibold">
								...
								<p className="font-mono font-bold text-text2 mt-1">
									{formatAmount(calcPlatformFee(selected))}
								</p>
							</div>
						</div>
					</div>
